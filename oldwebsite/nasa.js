...
var mykey = config.MY_NASAKEY;
var url_root = 'https://images-api.nasa.gov/';

var search_query = "apollo";
var url = url_root + "search?q=" + search_query;

$.getJSON(url, function(data){
  var num = data.collection.metadata.total_hits;
  var pics = data.collection.items;
  
  for(var i = 0; i < pics.length; i++)(function(i){
    if(pics[i].links.length > 0){
      var img_url = pics[i].links[0].href;
      var type = pics[i].data[0].media_type;
      if(type == "image"){
        var img = document.createElement('img');
        img.src = img_url;
        document.getElementById('populate_me').appendChild(img);
        console.log(type);
      }
    }
